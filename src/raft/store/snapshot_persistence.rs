...
const SNAPSHOT_FILE_SUFFIX: &str = ".bin.zst";
...
impl SnapshotPersistence {
    ...
    pub(crate) fn store(&self, snapshot: &StoredSnapshot) -> std::io::Result<()> {
        ...
        let file_name = format!(
            "{SNAPSHOT_FILE_PREFIX}{}_{timestamp}{SNAPSHOT_FILE_SUFFIX}",
    ...
    pub(crate) fn load_latest(&self) -> anyhow::Result<LoadLatestResult> {
        ...
        if had_files {
            warn!(
                ...
                "all discovered snapshots are unreadable; startup will fail to avoid data loss"
            );
            return Err(anyhow!(
                "no valid snapshot could be loaded from {:?}; refusing to start with empty state",
                self.dir
            ));
        }
    ...
    fn prune_file_set(&self, mut files: Vec<(PathBuf, u64)>) -> std::io::Result<()> {
        ...
        let to_delete = files.len().saturating_sub(self.retention);
    ...
    pub(crate) fn collect_snapshot_files(&self) -> std::io::Result<Vec<(PathBuf, u64)>> {
        archive::collect_indexed_files(&self.dir, SNAPSHOT_FILE_PREFIX, SNAPSHOT_FILE_SUFFIX)
    }

    pub(crate) fn collect_log_store_files(&self) -> std::io::Result<Vec<(PathBuf, u64)>> {
        archive::collect_indexed_files(
            &self.dir,
    ...
    pub(crate) fn extract_snapshot_idx(path: &Path) -> Option<u64> {
        archive::extract_idx_from_path(path, SNAPSHOT_FILE_PREFIX, SNAPSHOT_FILE_SUFFIX)
    }
    ...
    fn archive_log_store(&self, snapshot: &StoredSnapshot, timestamp: &str) -> std::io::Result<()> {
        ...
        write_atomic(&archive_path, &data)?;
        Ok(())
    }
...
#[cfg(test)]
#[path = "../tests/store_snapshot_id.rs"]
