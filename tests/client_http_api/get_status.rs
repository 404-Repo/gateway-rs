...
use crate::support::{
    HOTKEY_SEED_BASE, add_failure_result, add_success_result, add_success_result_with_worker_id,
    add_task_prompt, build_harness, hotkey_from_seed, read_response,
};
...
#[tokio::test]
async fn get_status_missing_api_key() {
    let h = build_harness().await;
...
#[tokio::test]
async fn get_status_invalid_api_key_format() {
    let h = build_harness().await;
    let task_id = add_task_prompt(&h, "robot", None).await;
    let res = TestClient::get(format!("http://localhost/get_status?id={task_id}"))
...
async fn get_status_failure() {
    ...
    let worker = hotkey_from_seed(HOTKEY_SEED_BASE + 3);
